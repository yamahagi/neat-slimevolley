...
from settings import ATTENUATION, DIRECTION_COEF, HIT_COEF
...
class Game:
    ...
    """
    def __init__(self, gameState, generation):
        self.baselineParams = initBaselinePolicyParams()
        self.generation = generation
        self.ground = None
        self.fence = None
        self.fenceStub = None
        self.attenuation_rate = 30
    ...
    def get_hit_bonus(self):
        return HIT_COEF * (ATTENUATION ** self.generation)
    
    def jump_hit_bonus(self, agent):
        is_jumping = agent.vy > 0
        is_airborne = agent.y > (REF_U + 0.2)
        return jnp.where(is_jumping & is_airborne, 0.1 * (ATTENUATION ** self.generation), 0.0)
    
    def step(self):
        """ main game loop """
        # 成功時ボーナス（指数減衰付き）
        def directional_hit_bonus(generation):
            return DIRECTION_COEF * (ATTENUATION ** generation)
        ...
        hit_reward_left = jnp.float32(0.0)
        hit_reward_right = jnp.float32(0.0)

        hit_reward_left += jnp.where(hit_left, self.get_hit_bonus(), 0.0)
        hit_reward_right += jnp.where(hit_right, self.get_hit_bonus(), 0.0)

        hit_success_left = hit_left & (self.ball.p.x > self.agent_left.p.x) & (self.agent_left.p.vx > 0)
        hit_success_right = hit_right & (self.ball.p.x < self.agent_right.p.x) & (self.agent_right.p.vx < 0)
        
        hit_reward_left += jnp.where(hit_success_left, directional_hit_bonus(self.generation) * (self.ball.p.vx * 0.5), 0.0)
        hit_reward_right += jnp.where(hit_success_right, directional_hit_bonus(self.generation) * (- self.ball.p.vx * 0.5), 0.0)

        return result, hit_reward_left, hit_reward_right
...
def update_state(action_right: jnp.ndarray, game_state: GameState, generation: int, key: jnp.array):
    game = Game(game_state, generation)
    game.setRightAction(action_right)
    game.setAction()

    base_reward, hit_reward_left, hit_reward_right = game.step()
    updated_game_state = game.getGameState()
    obs_left = game.agent_left.getObservation()
    obs_right = game.agent_right.getObservation()

    agent_right = game.agent_right.p
    ball = game.ball.p
    distance = jnp.abs(agent_right.x - ball.x)

    
    def distance_penalty(d):
        return 0.2 * d

    # base_reward == -1（自分が失点）のときのみペナルティ + ボーナス付加
    lambda_ = 0.9 * (ATTENUATION ** generation)
    adjusted_reward_right = jnp.where(
        base_reward == -1,
        lambda_ * (-distance_penalty(distance)) + (1.0 - lambda_) * base_reward,
        base_reward
    )

    updated_game_state = update_state_for_new_match(updated_game_state, base_reward, key)
    return updated_game_state, base_reward, adjusted_reward_right + hit_reward_left, adjusted_reward_right + hit_reward_right, obs_left, obs_right


def update_state_bothside(action_left: jnp.ndarray, action_right: jnp.ndarray, game_state: GameState, generation: int, key: jnp.array):
    game = Game(game_state, generation)
    game.setLeftAction(action_left)
    game.setRightAction(action_right)
    game.setAction()
    # reward, bonus_reward_left, bonus_reward_right = game.step() # from perspective of the agent on the right
    base_reward, hit_reward_left, hit_reward_right = game.step() # from perspective of the agent on the right
    updated_game_state = game.getGameState()
    obs_left = game.agent_left.getObservation()
    obs_right = game.agent_right.getObservation()
    
    agent_left = game.agent_left.p
    ball = game.ball.p
    distance_left = jnp.abs(agent_left.x - ball.x)
    
    agent_right = game.agent_right.p
    ball = game.ball.p
    distance_right = jnp.abs(agent_right.x - ball.x)
    ...
    k = 5.0  # 例: 早めにbase_rewardに移行させたい場合は小さめ（例: 15〜30）
    # base_reward == 1（自分が失点）のときのみペナルティ + ボーナス付加
    lambda_ = jnp.exp(-generation / k)
    adjusted_reward_left = jnp.where(
        base_reward == 1,
        lambda_ * (-distance_penalty(distance_left)) - (1.0 - lambda_) * base_reward,
        - base_reward
    )
    # base_reward == -1（自分が失点）のときのみペナルティ + ボーナス付加
    adjusted_reward_right = jnp.where(
        base_reward == -1,
        lambda_ * (-distance_penalty(distance_right)) + (1.0 - lambda_) * base_reward,
        base_reward
    )

    updated_game_state = update_state_for_new_match(
        updated_game_state, base_reward, key)

    return updated_game_state, base_reward, adjusted_reward_left + hit_reward_left, adjusted_reward_left + hit_reward_right, obs_left, obs_right
...
def get_obs(game_state: GameState, generation: int):
    game = Game(game_state, generation)
    return game.agent_right.getObservation(), game.agent_left.getObservation()
...
class SlimeVolley(VectorizedTask):
    ...
    def __init__(self,
                 max_steps: int = 3000,
                 test: bool = False,
                 is_left_baseline: bool = False):

        self.max_steps = max_steps
        self.obs_shape = tuple([12, ])
        self.act_shape = tuple([3, ])
        self.test = test
        self.is_left_baseline = is_left_baseline

        def reset_fn(key, generation):
            next_key, key = random.split(key)
            game_state = get_init_game_state_fn(key)
            return State(game_state=game_state, obs=get_obs(game_state, generation),
                             steps=jnp.zeros((), dtype=int), key=next_key)

        self._reset_fn = jax.jit(jax.vmap(reset_fn))
        
        def step_fn(state, action_left, action_right, generation):
            next_key, key = random.split(state.key)
            if self.is_left_baseline:
                cur_state, base_reward, weighted_score_left, weighted_score_right, obs_left, obs_right = update_state(action_right=action_right, game_state=state.game_state, generation=generation, key=key)
            else:
                cur_state, base_reward, weighted_score_right, weighted_score_right, obs_left, obs_right = update_state_bothside(action_left=action_left, action_right=action_right, game_state=state.game_state, generation=generation, key=key)
            steps = state.steps + 1
            ...
            return State(game_state=cur_state, obs=(obs_left, obs_right),
                             steps=steps, key=next_key), base_reward, weighted_score_right, weighted_score_right, done
        self._step_fn = jax.jit(jax.vmap(step_fn))

    def reset(self, key: jnp.ndarray, generation: jnp.array) -> State:
        return self._reset_fn(key, generation)

    def step(self, state: State, action_left: jnp.ndarray, action_right: jnp.ndarray, generation: jnp.array) -> Tuple[State, jnp.ndarray, jnp.ndarray]:
        return self._step_fn(state, action_left, action_right, generation)
    ...
    def render(state: State, task_id: int = 0) -> Image:
        ...
        game = Game(task_game_state, generation=0)
        canvas = game.display()
